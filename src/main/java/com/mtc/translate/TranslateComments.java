...
public class TranslateComments {
    ...
    private String[] tableNames = {"OIGE","OIGN","OPDN","ORPD","ODLN","ORDN","OWTR"};

    public TranslateComments(){
    ...
    public void translate(){
        Arrays.stream(tableNames).forEach(tableName->{
            List<Map<String,Object>> list = baseDaoByHana.findResultHana(tableName,"SBO_AG_FARM");
            translate(list,tableName,null,null);
        });
    ...
    private void translate(List<Map<String,Object>> list,String tableName,String key,String express){
        list.stream().forEach(map->{
            String beforeFarmName = map.get("Comments") == null ? "":map.get("Comments").toString();
            ...
            if(beforeFarmName.equals("安徽省地质实验研究所(国土资源部合肥矿产资源监督检测中心）")){
               ...
               if(StringUtils.isNotEmpty(beforeFarmName)){
                   ...
                   setMap.put("Comments",afterFarmName);

                   Map<String,Object> whereMap = new HashMap<>();
                   whereMap.put("DocEntry",map.get("DocEntry"));
                   baseDaoByHana.update("SBO_AG_FARM",tableName,setMap,whereMap);
               }
    ...
    public static void main(String[] args) {
        TranslateComments oprc = new TranslateComments();
        oprc.translate();
        //Pullet青年鸡场1号第1栋
        //Pullet 3# House 1#
    }
